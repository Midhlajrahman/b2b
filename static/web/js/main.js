...
//     const popupWindow = el.querySelector('.js-popup-window');
//     const closeButton = el.querySelector('.js-close-button'); // Close button
...
//         const title = option.querySelector('.js-search-option-target').textContent;
...
//   function showList(searchTerm, results) {
...
//     // Populate results with matching cities
//     filteredCities.forEach(city => {
//       const option = document.createElement('div');
//       option.classList.add('js-search-option', 'd-block', 'col-12', 'text-left', 'rounded-4', 'px-20', 'py-15');
//       option.innerHTML = `
//         <div class="d-flex">
...
function liveSearch() {
  ...
  targets.forEach(el => {
    ...
    const selectOption = (title) => {
      console.log(`Option selected: ${title}`); // Debugging line
      search.value = title;
      popupWindow.classList.remove('-is-active');
      results.innerHTML = ''; // Clear results after selection
    };
    ...
    // Function to show filtered list based on search term
    function showList(searchTerm, results) {
      results.innerHTML = ''; // Clear previous results
      ...
      filteredCities.forEach(city => {
        ...
        option.addEventListener('click', () => {
          selectOption(city.name);
          selectCity(city.name); // Ensure this function is called as well
        });
...
// Make sure this function is outside of liveSearch, so it can be accessed globally
function selectCity(cityName) {
  console.log(`Selected city for input: ${cityName}`); // Debugging line
  var inputField = document.getElementById('location-input');
  if (inputField) {
    inputField.value = cityName;
  } else {
    console.error('Input field not found!'); // Error handling if input field is missing
  }
}

// Initialize live search
document.addEventListener('DOMContentLoaded', () => {
  liveSearch(); // Initialize after the DOM is fully loaded
});
...
function calendarInteraction() {
  ...
  target.forEach(elTarget => {
    ...
    gridCells.forEach((el, i) => {
      el.addEventListener('click', () => {
        console.log('Clicked cell:', el); // Debug: Log the clicked cell
        ...
        if (completeState) {
          ...
          // Update the display for the selected dates
          const firstDateValue = `${firstItem.getAttribute('data-week')} ${firstItem.querySelector('.js-date').innerHTML} ${firstItem.getAttribute('data-month')}`;
          const lastDateValue = `${lastItem.getAttribute('data-week')} ${lastItem.querySelector('.js-date').innerHTML} ${lastItem.getAttribute('data-month')}`;

          console.log('First Date:', firstDateValue); // Debug: Log the first date value
          console.log('Last Date:', lastDateValue); // Debug: Log the last date value

          firstDate.innerHTML = firstDateValue; // Display first selected date
          lastDate.innerHTML = lastDateValue; // Optionally display the last selected date
